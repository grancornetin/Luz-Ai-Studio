// CloneImageModule.tsx - COMPLETO Y REPARADO (sin errores de tipo)
...
export default function CloneImageModule() {
  ...
  async function handleGenerateBase() {
    ...
    try {
      // Aseguramos que los valores no sean null antes de pasarlos
      const safeTarget = normalizeImageInput(targetImage);
      const safeFace = normalizeImageInput(face1);
      const safeBody = normalizeImageInput(body1);
      if (!safeTarget || !safeFace || !safeBody) {
        throw new Error("Faltan referencias obligatorias");
      }
  ...
  async function handleApplyOutfitsAndProducts() {
    ...
    try {
      // Aseguramos los valores obligatorios
      const safeTarget = normalizeImageInput(baseComposition);
      const safeFace = normalizeImageInput(face1);
      const safeBody = normalizeImageInput(body1);
      if (!safeTarget || !safeFace || !safeBody) {
        throw new Error("Faltan referencias base para aplicar cambios");
      }
