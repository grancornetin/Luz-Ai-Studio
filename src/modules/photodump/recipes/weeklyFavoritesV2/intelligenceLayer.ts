 ...
 *
 * Conecta esta receta con los dos motores que existen justamente para que
 * las fotos no se vean genéricas:
 *
 *  - HPI (Human Performance Intelligence): pose, gesto, expresión, energía
 *    corporal. Solo aporta esto — nunca decide qué referencia usar, qué
 *    item aparece, ni cobertura. Ver src/services/hpiService.ts.
...
export function applyIntelligence(
  ...
  const hpiBlock     = hasPerson ? buildHpiBlock(hpiConfigForShot(contract, gender)) : '';
  const hpiNegatives = hasPerson ? getHpiNegatives(gender) : [];
