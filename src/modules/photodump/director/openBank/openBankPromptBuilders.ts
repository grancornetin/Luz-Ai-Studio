...
// BUG REAL corregido (prueba 27, 27 ago 2026): investigación de causa de fondo
// pedida explícitamente por el usuario tras 27 pruebas sin lograr un set
// consistente de punta a punta — no un parche puntual más. Se encontró que
// Finding 005 (recipes/manifiesto de dirección/10_experimental_findings_001.md,
// validado julio 2026 con 8 ejemplos reales comparados) ya identificó que el
// lenguaje de "camera roll" (líneas de arriba, Finding 004) arregla la
// TEXTURA de la foto pero no su COMPOSICIÓN — sin una capa aparte, el modelo
// sigue componiendo la escena como campaña editorial de moda (sujeto
// centrado, mirada directa y posada a cámara, fondo perfectamente enmarcado
// detrás de la cabeza) aunque la textura de celular ya sea correcta. Este
// bloque (UGC_CASUAL_COMPOSITION_BLOCK en recipes/shared.ts) ya está
// implementado y validado en outfitMultiLook — nunca se portó a open_bank,
// pese a que el síntoma exacto (poses "tipo campaña"/demasiado posadas) se
// repitió en el feedback de múltiples pruebas reales de este pipeline.
...
1. NUNCA describas ninguna prenda, color de ropa, o cómo cae/se acomoda la ropa del candidato del banco — el outfit lo resuelve la imagen de referencia del usuario, nunca texto describiendo ropa de otra persona. Esto incluye NO INVENTAR prendas nuevas que no están en ninguna referencia ni en el candidato (bug real confirmado, prueba 14 ago 2026: un shot mencionó "her jacket slips" describiendo una chaqueta que no aparecía en ninguna referencia de outfit ni en el candidato del banco elegido — el generador de imágenes la agregó porque el texto se la pidió). Si necesitás describir que se ve piel/un hombro/una prenda deslizándose para la pose, primero verificá que esa prenda exista en la referencia real del usuario (mirá el richDetailBlock/outfit real, no lo asumas) — si no podés confirmarlo, no lo menciones y dejá que la imagen de referencia del outfit resuelva qué prendas hay.
   MISMA REGLA APLICA A ACCESORIOS HEREDADOS DEL CANDIDATO, NO SOLO PRENDAS (bug real confirmado, prueba 27 ago 2026: un selfie de auto de noche salió con lentes de sol puestos — ningún texto los pedía explícitamente, pero el candidato de referencia del banco los tenía, y además es implausible llevar lentes oscuros de noche). Antes de heredar cualquier objeto del candidato que no sea pose/gesto/encuadre (joyas puntuales, lentes, sombreros, accesorios de pelo), preguntate: (a) ¿está confirmado en alguna referencia real del usuario?, y (b) ¿tiene sentido lógico en el momento real de la noche descrito en el brief (hora, clima, contexto)? Si no podés confirmar ambas, no lo menciones ni lo heredes — va a discardedElements.
...
5. MECÁNICA FÍSICA DE CÓMO SE TOMÓ LA FOTO (bug real confirmado 2 veces, en 2 direcciones opuestas — no es un detalle menor: primero un gesto posado se redactó con ángulo de selfie sin ninguna cámara real; después un MIRROR SELFIE real se redactó "sin mostrar el teléfono", dejando el reflejo del espejo sin explicación de cómo se tomó, porque esa instrucción es de la lectura equivocada). Cualquier gesto donde la mano de la protagonista esté cerca de su propio rostro, O cualquier shot que use un espejo/reflejo, es AMBIGUO por defecto — nunca lo dejes sin resolver. Primero preguntate cuál de estas 3 lecturas es más creíble para ESTE momento puntual de la noche (no hay una respuesta fija, depende del contexto de cada shot: ¿hay compañía real que podría estar tomando la foto? ¿es un momento íntimo/solitario donde una selfie es lo más natural? ¿el shot ya usa un espejo/reflejo del venue?), y describila sin ambigüedad:
   a) Selfie de brazo extendido (SIN espejo en el encuadre): SIEMPRE mostrá el teléfono en cuadro, sostenido por su propia mano cerca del rostro — es lo que físicamente explica el encuadre y lo que distingue una selfie real de una foto tomada por otra persona (bug real confirmado, prueba 3 sep 2026, receta outfit_check: un selfie de auto redactado SIN el teléfono en cuadro — solo "ángulo elevado" en texto, sin el objeto — resultó en una foto de tercero: encuadre amplio mostrando el tablero completo y a ella de perfil, sin ningún teléfono visible, exactamente la perspectiva de alguien más sacándole la foto desde el asiento de al lado). El teléfono en la mano ES el ancla física que fuerza al generador a un encuadre cercano — omitirlo es lo que produce el plano de tercero, no lo evita. Describí explícitamente: el teléfono (color/modelo genérico, ej. "dark gray smartphone") sostenido con el brazo/codo DOBLADO y cerca del propio cuerpo/rostro, visible en el borde del encuadre — nunca el brazo extendido lejos hacia el frente.
      ...
      07_manual_revalidation_protocol.md — nunca antes traducida a
      instrucción de prompt; bug real confirmado repetidas veces en selfies
      de auto: el resultado se lee como si otra persona la fotografiara
      desde el asiento de al lado, mostrando su cuerpo entero de perfil, no
      como algo que ella misma se sacó con su propio brazo). Antes de cerrar
      la descripción de un selfie de brazo, verificá mentalmente: "si un
      brazo humano real sostuviera el teléfono con el codo doblado cerca del
      cuerpo, ¿el encuadre resultante mostraría ESTO?" — un selfie de brazo
      real NUNCA muestra el cuerpo completo, ni a la persona de perfil
      completo, ni el interior completo de un auto/habitación desde una
      distancia amplia; el encuadre queda necesariamente apretado y cercano
      (rostro + hombros + como mucho torso superior, CON el teléfono visible
      en el borde del encuadre sosteniendo la toma). Si tu descripción
      incluye piernas, un ángulo de perfil completo, "mostrando el interior
      del auto/venue" como parte central del encuadre, O NO MENCIONA EL
      TELÉFONO EN CUADRO, no es un selfie de brazo real — es una perspectiva
      de tercero sin nadie que la tome. Reescribí el encuadre como un plano
      cerrado real, con el teléfono visible, antes de continuar.
...
PROHIBIDO EL ELEMENTO "O ALTERNATIVO" EN LA DESCRIPCIÓN DE VENUE (bug real confirmado 2 veces, pruebas 14 y 27 ago 2026 — la 2da vez sobre el mismo tipo de elemento, una baranda: "the rooftop railing is glass or dark metal", y en ese mismo shot la mano apoyada en la baranda terminó sin ningún apoyo visible, reforzando que esta ambigüedad no es cosmética, produce fallas geométricas reales): un shot final redactó el fondo como "a city view or a railing" — la palabra "or" es la señal de que no se decidió qué hay realmente ahí, dejándoselo "a elección" al generador de imágenes, que entonces puede inventar cualquiera de las 2 opciones sin anclarla a nada de los shots anteriores. El finalPrompt describe UN lugar concreto, nunca una posibilidad entre varias — si el shot necesita continuidad de venue, elegí explícitamente uno solo de los elementos ya establecidos (o el que la pose realmente necesita) y describilo sin alternativas. Nunca escribas "X or Y", "either X or Y", "possibly X" en la descripción del venue o del mobiliario.
CHEQUEO MECÁNICO FINAL: antes de entregar el finalPrompt, releelo buscando literalmente la palabra " or " en cualquier descripción de objeto/material/mobiliario/venue — si aparece, no es una ambigüedad de estilo, es una decisión no tomada; elegí una sola opción concreta y reescribí esa frase antes de responder.
