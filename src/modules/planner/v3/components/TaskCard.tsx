  ...
  return <article className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
    ...
    {open && <div className="space-y-5 border-t border-slate-100 p-4 text-sm sm:p-5">
      ...
      <section><h4 className="font-black text-slate-900">🛠 Cómo crearlo</h4><p className="mt-1 text-slate-600"><b>{MODULE_DISPLAY_NAMES[task.toolModule]}:</b> {task.toolReason}</p>
        ...
        {task.prompt && <div className="mt-3 rounded-2xl bg-slate-50 p-3"><p className="mb-2 text-xs font-bold text-slate-700">Descripción lista para usar</p><div className="mb-2 flex justify-end"><CopyButton text={task.prompt} /></div><p className="whitespace-pre-wrap text-xs text-slate-600">{task.prompt}</p></div>}
      </section>
      <section><h4 className="font-black text-slate-900">✍️ Listo para publicar</h4><div className="mt-2 rounded-2xl bg-slate-50 p-3"><CopyButton text={`${task.caption}\n\n${task.hashtags}`} /><p className="mt-2 whitespace-pre-wrap text-slate-700">{task.caption}</p><p className="mt-2 text-xs text-[#F72C5B]">{task.hashtags}</p></div><p className="mt-2 text-slate-600"><b>Para recibir más comentarios:</b> {task.engagementHook}</p></section>
