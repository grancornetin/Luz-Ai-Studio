...
const ProductPhotography: React.FC<ProductPhotographyProps> = ({ saveProduct, products, standalone }) => {
  ...
  const generateImageWithAllRules = async (intent: string, refs: string[]) => {
    ...
    return geminiService.generateImage(
      ...
      '1K',
      refs,
      "3:4" // Explicitly pass aspectRatio
    );
  };
    ...
    <div className="max-w-7xl mx-auto space-y-6 md:space-y-8 pb-20 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-1">
        ...
        <div className={`flex bg-white p-1 rounded-2xl shadow-sm border border-slate-100 transition-all ${isGenerating ? 'opacity-50 pointer-events-none' : ''}`}>
          <button onClick={() => { setActiveTab('create'); window.scrollTo(0,0); resetCreator(); }} className={`px-5 md:px-8 py-2 md:py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'create' ? 'bg-accent-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-700'}`}>Laboratorio</button>
          <button onClick={() => { setActiveTab('library'); window.scrollTo(0,0); resetCreator(); }} className={`px-5 md:px-8 py-2 md:py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'library' ? 'bg-accent-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-700'}`}>Catálogo ({products.length})</button>
        </div>
      </header>
      ...
      {activeTab === 'create' ? (
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className={`bg-white p-6 md:p-8 rounded-[32px] md:rounded-[40px] shadow-sm border border-slate-100 space-y-8 transition-all ${isGenerating ? 'opacity-40 pointer-events-none' : ''}`}>
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex bg-slate-100 p-1 rounded-xl">
                      <button onClick={() => setStyle('comercial')} className={`flex-1 py-2 text-xs font-black uppercase rounded-lg transition-all ${style === 'comercial' ? 'bg-white shadow-sm text-accent-600 border border-accent-50' : 'text-slate-400'}`}>Comercial</button>
                      <button onClick={() => setStyle('organico')} className={`flex-1 py-2 text-xs font-black uppercase rounded-lg transition-all ${style === 'organico' ? 'bg-white shadow-sm text-accent-600 border border-accent-50' : 'text-slate-400'}`}>Orgánico</button>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-4 gap-2 md:gap-3">
                    {files.map((f, i) => (
                      <div key={i} className="aspect-square relative rounded-xl overflow-hidden border border-slate-200 group">
                        ...
                        <button onClick={() => removeFile(i)} className="absolute top-1 right-1 w-5 h-5 bg-red-500 text-white rounded-full flex items-center justify-center text-[10px] opacity-0 group-hover:opacity-100 transition-opacity"><i className="fa-solid fa-trash"></i></button>
                      </div>
                    ))}
                    {files.length < 4 && (
                      <label className="aspect-square bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:bg-slate-100 transition-all group">
                        <i className="fa-solid fa-camera text-slate-300 group-hover:scale-110 transition-transform"></i>
                ...
                <button onClick={startGeneratingHero} disabled={isGenerating || files.length === 0 || !name} className="w-full py-4 md:py-5 bg-accent-600 text-white rounded-2xl md:rounded-[24px] font-black text-xs uppercase tracking-[0.2em] shadow-xl shadow-accent-100 hover:bg-accent-700 active:scale-95 transition-all disabled:opacity-50">
            ...
            <div className="bg-slate-900 rounded-[40px] md:rounded-[56px] p-6 md:p-10 min-h-[500px] md:min-h-[750px] flex flex-col shadow-2xl relative overflow-hidden">
              {(currentStep === 'setup' || currentStep === 'analyzing') && (
                <div className="flex-1 flex flex-col items-center justify-center text-center space-y-6 md:space-y-10 p-4">
                   ...
                   {currentStep === 'analyzing' && <p className="text-accent-400 text-xs font-black uppercase tracking-widest animate-pulse">{processingStatus}</p>}
                </div>
              )}

              {(currentStep === 'generating_hero' || currentStep === 'generating_remaining') && (
                <div className="flex-1 flex flex-col items-center justify-center text-center space-y-6 md:space-y-10 p-4">
                  <div className="w-16 h-16 border-4 border-white/5 border-t-accent-500 rounded-full animate-spin"></div>
                  <p className="text-accent-400 text-xs font-black uppercase tracking-widest animate-pulse">{processingStatus}</p>
                </div>
              )}
                ...
                <div className="flex-1 flex flex-col space-y-8 animate-in fade-in">
                  <header className="flex justify-between items-center border-b border-white/10 pb-6">
                    <div>
                      ...
                      <p className="text-accent-400 text-xs font-black uppercase tracking-[0.3em] mt-1 italic">Validación de Fidelidad 1:1</p>
                    </div>
                  </header>
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
                    <div className="md:col-span-7 flex flex-col items-center justify-center">
                      <div className="w-full space-y-6">
                        <div className="aspect-[3/4] bg-slate-800 rounded-[40px] overflow-hidden shadow-2xl border-4 border-white/10 relative group">
                        ...
                        <div className="flex gap-4">
                          ...
                          <button 
                            ...
                            className={`flex-1 py-5 rounded-3xl font-black text-xs uppercase shadow-2xl transition-all ${isGenerating ? 'bg-slate-700 text-slate-500' : 'bg-accent-500 text-white hover:bg-accent-400'}`}
                          >
                            Aprobar y Continuar
                          </button>
                        </div>
                    ...
                    <div className="md:col-span-5 bg-white/5 rounded-[40px] border border-white/10 p-6 space-y-6 text-white">
                      <h4 className="text-xs font-black text-accent-400 uppercase tracking-widest border-b border-white/10 pb-4">Guía de Aprobación</h4>
                      <p className="text-xs font-bold leading-relaxed">
                        <i className="fa-solid fa-circle-check text-accent-400 mr-2"></i>
                ...
                <div className="space-y-6 md:space-y-8 animate-in fade-in">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-white/10 pb-6 md:pb-8 gap-4">
                    <div>
                      ...
                      <span className="text-accent-400 text-xs font-black uppercase tracking-[0.2em]">{processingStatus}</span>
                    </div>
                    <div className="flex gap-2 w-full sm:w-auto">
                      <button onClick={() => handleDownloadZip(null)} className="flex-1 sm:flex-none px-4 md:px-6 py-3 md:py-4 bg-white/10 text-white rounded-xl md:rounded-2xl text-xs font-black uppercase flex items-center justify-center gap-2 hover:bg-white/20 transition-all border border-white/5">
                        <i className="fa-solid fa-file-zipper"></i> zip completo
                      </button>
                      <button onClick={handleSaveToCatalog} className="flex-1 sm:flex-none px-6 md:px-8 py-3 md:py-4 bg-accent-500 text-white rounded-xl md:rounded-2xl text-xs font-black uppercase shadow-lg hover:bg-accent-600 transition-all active:scale-95">
                        guardar en catálogo
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 md:gap-6">
                    {generatedShots.map((p, i) => (
                      <div key={i} className={`group relative aspect-[3/4] rounded-[24px] md:rounded-[32px] overflow-hidden bg-white shadow-2xl ${i === 0 ? 'col-span-2 aspect-[16/9]' : ''}`}>
                        <img src={p} className="w-full h-full object-cover" />
                        <button onClick={() => handleDownloadIndividual(p, `${name.replace(/\s+/g, '_')}_shot_${i+1}.png`)} className="absolute bottom-4 right-4 md:bottom-8 md:right-8 w-10 h-10 md:w-12 md:h-12 bg-white text-slate-900 rounded-full flex items-center justify-center shadow-xl opacity-0 group-hover:opacity-100 transition-all hover:scale-110">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10 px-1">
          {products.map(product => (
            <div key={product.id} className="bg-white p-5 md:p-6 rounded-[40px] md:rounded-[56px] border border-slate-100 shadow-sm hover:shadow-2xl transition-all group overflow-hidden cursor-pointer" onClick={() => setSelectedProduct(product)}>
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-[10000] bg-black/95 flex items-center justify-center p-4 md:p-6 animate-in fade-in duration-300">
           <div className="bg-white w-full max-w-6xl h-[95vh] md:h-[90vh] rounded-[32px] md:rounded-[56px] overflow-hidden flex flex-col md:flex-row shadow-2xl relative animate-in zoom-in duration-300">
              ...
              <div className="w-full md:w-1/2 h-1/2 md:h-full bg-slate-50 p-6 md:p-10 overflow-y-auto custom-scrollbar">
                 <div className="grid grid-cols-2 gap-4">
              ...
              <div className="w-full md:w-1/2 h-1/2 md:h-full p-6 md:p-12 overflow-y-auto space-y-6 md:space-y-10 custom-scrollbar">
                 ...
                 <p className="text-slate-600 font-medium leading-relaxed italic border-l-4 border-accent-500 pl-4 md:pl-6 text-xs md:text-base">"{selectedProduct.commercialDescription}"</p>
                 
                 <div className="flex gap-3">
                   <button 
                     onClick={() => copyToClipboard(selectedProduct.technicalDescription, "Ficha técnica copiada!")}
                     className="flex-1 py-4 bg-slate-100 text-slate-800 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-slate-200 transition-all active:scale-95"
                   >
                     <i className="fa-solid fa-copy mr-2"></i>Copiar Ficha Técnica
                   </button>
                   <button 
                     onClick={() => copyToClipboard(selectedProduct.commercialDescription, "Copy comercial copiado!")}
                     className="flex-1 py-4 bg-slate-100 text-slate-800 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-slate-200 transition-all active:scale-95"
                   >
                     <i className="fa-solid fa-copy mr-2"></i>Copiar Copy Comercial
                   </button>
                 </div>

                 <button onClick={() => handleDownloadZip(selectedProduct)} className="w-full py-5 bg-accent-600 text-white rounded-[24px] font-black text-xs uppercase tracking-widest shadow-xl hover:bg-accent-700 transition-all active:scale-95">
                   <i className="fa-solid fa-file-zipper mr-2"></i> Descargar Pack Completo (.zip)
                 </button>
              </div>
           </div>
        </div>
