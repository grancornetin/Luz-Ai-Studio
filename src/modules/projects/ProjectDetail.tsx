...
const ProjectDetail: React.FC = () => {
  ...
  // Callbacks del copiloto — actualizan el proyecto local sin recargar
  ...
  const handleCalendarUpdate = (calendar: CalendarEntry[]) => {
    ...
    // Si hay calendario y estamos en tab copiloto, ir a calendario
    setMobileTab('calendar');
  };

  if (loading) return (
    <div className="flex items-center justify-center min-h-[400px] text-slate-400 text-sm font-bold uppercase tracking-widest animate-pulse">
      Cargando proyecto...
    </div>
  );
  ...
  const ImagesPanel = () => (
    <div className="space-y-5">
      <label className={`flex items-center justify-center gap-2 w-full py-3 rounded-xl border-2 border-dashed transition-all cursor-pointer ${
        uploading ? 'border-indigo-300 bg-indigo-50 text-indigo-400' : 'border-slate-200 hover:border-indigo-300 hover:bg-indigo-50 text-slate-400 hover:text-indigo-500'
      }`}>
        <input type="file" accept="image/*" className="hidden" onChange={handleUploadReference} disabled={uploading} />
        {uploading
          ? <><div className="w-4 h-4 border-2 border-indigo-400 border-t-transparent rounded-full animate-spin" /><span className="text-xs font-bold uppercase tracking-widest">Subiendo...</span></>
          : <><Upload className="w-4 h-4" /><span className="text-xs font-bold uppercase tracking-widest">Subir referencia al proyecto</span></>
        }
      </label>
      ...
      {project.items.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-slate-400 text-xs mt-1">Subí una referencia o usá el copiloto para generar contenido</p>
        </div>
      )}
    </div>
  );
  ...
  const TABS: { id: WorkspaceTab; label: string; icon: React.ReactNode; badge?: number }[] = [
    ...
    { id: 'calendar', label: 'Plan',     icon: <CalendarDays className="w-4 h-4" />, badge: hasCalendar ? (project.calendar?.filter(e => e.status === 'done').length ?? 0) || undefined : undefined },
  ];
    ...
    <div className="flex flex-col h-full max-h-screen">
      ...
      <header className="flex items-center gap-3 px-4 py-4 bg-white border-b border-slate-100 flex-shrink-0 flex-wrap gap-y-2">
        ...
        {editingName ? (
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <h1 className="text-xl sm:text-2xl font-black uppercase italic tracking-tighter text-slate-900 truncate">{project.name}</h1>
      ...
      <div className="flex md:hidden border-b border-slate-100 bg-white flex-shrink-0">
        {TABS.map(tab => (
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 py-3 text-xs font-black uppercase tracking-widest transition-colors border-b-2 relative ${
              mobileTab === tab.id ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-400'
            }`}
          >
            {tab.icon}
            {tab.label}
            {tab.badge !== undefined && (
              <span className="bg-slate-200 text-slate-600 text-[9px] font-black px-1.5 py-0.5 rounded-full ml-0.5">
                {tab.badge}
              </span>
            )}
          </button>
      ...
      <div className="flex-1 overflow-hidden">
        ...
        <div className="hidden md:grid md:grid-cols-12 h-full">
          ...
          {/* Imágenes (30%) */}
          <div className="md:col-span-3 p-5 overflow-y-auto border-r border-slate-100 bg-slate-50/50">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-4">Imágenes del proyecto</p>
            <ImagesPanel />
          </div>

          {/* Copiloto (45%) */}
          <div className="md:col-span-5 bg-slate-900 overflow-hidden flex flex-col border-r border-white/5" style={{ minHeight: 0 }}>
            <ProjectCopilot
              project={project}
              onChecklistUpdate={handleChecklistUpdate}
              onCalendarUpdate={handleCalendarUpdate}
          ...
          {/* Calendario + Plan (25%) */}
          <div className="md:col-span-4 p-5 overflow-y-auto bg-slate-900/50 border-l border-white/5">
            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-4">Plan de contenido</p>
            <ProjectCalendar
              projectId={project.id}
              calendar={project.calendar ?? []}
              checklist={project.checklist ?? []}
