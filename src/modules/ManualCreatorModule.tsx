...
const ManualCreatorModule: React.FC<ManualCreatorModuleProps> = ({ onSave }) => {
  ...
  const handleCreate = async () => {
    ...
    try {
      // Construct identity prompt with all user selections
      ...
      const shots = await avatarService.generateMasterSet(
        ...
        data.gender, 
        data.outfit,
        data.personality, // Pass personality for dynamic pose instruction
        data.expression   // Pass expression for dynamic facial expression instruction
      );
      ...
      // Update physicalDescription to include personality and expression
      ...
      const newAvatar: AvatarProfile = {
        ...
        baseImages: shots,
        metadata: data as any, // metadata still includes all data properties
        createdAt: Date.now()
      };
