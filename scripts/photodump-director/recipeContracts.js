/**
 * scripts/photodump-director/recipeContracts.js
 *
 * Punto C del diagrama del usuario ("idea cruda de director creativo.txt"):
 * el contrato de cada receta — de qué se trata la historia, qué shots
 * necesita, qué rol cumple cada uno. Para outfit_night_out esto YA EXISTE
 * en producción (src/modules/photodump/recipes/outfitNightOut/nightMoments.ts
 * + levelResolver.ts) — acá solo se describe en texto simple para que el
 * director de esta prueba (Fase A, standalone) lo tenga como contexto fijo,
 * sin reimplementar la lógica de selección real de producción.
 *
 * Si mañana se agregan más recetas al director, cada una necesita su propia
 * entrada acá (o su propio archivo), siguiendo la misma forma.
 */
...
ella en ese momento" — nunca "así se ve la prenda".`,
    ...
    // no es parte del banco rotable de night moments. Se declara como su
    // propio tipo para que el director nunca lo confunda con posed_portrait
    // (bug real detectado: sin esto, Gemini reusaba el id posed_portrait
    // tanto para el mirror check como para un momento de noche distinto).
    fixedShotTypes: [
      {
        id: 'mirror_check',
        ...
        // salir de noche", rompiendo la lógica temporal del set. El mirror
        // check pasa SIEMPRE poco antes de salir — su iluminación debe
        // coincidir con la hora real del evento del brief (noche → luz
        // artificial/nocturna; almuerzo → luz de día; tarde → golden hour),
        // NUNCA con la iluminación del candidato del banco elegido como
        // inspiración de pose.
...
export function getRecipeContract(recipeId) {
