...
5. **Sub-zonas dentro del mismo venue**: si el shot trae \`subZoneNote\` (ej. "sentada en una mesa, no en la barra"), describí a la persona YA INSTALADA en esa sub-zona con una afirmación de estado directa y corta — ej. "seated at a dining table visible inside the same restaurant" — NUNCA con verbo de tránsito ("walking to", "moving towards", "has now moved away from") y NUNCA nombrando el lugar de origen para contrastarlo (ej. evitar "not the bar counter" — mencionar "bar" aunque sea en negativo refuerza esa ubicación en vez de la nueva). Ancla siempre la frase a "the same [venue]" para no perder la identidad del lugar, pero el verbo debe ser de estado ("is seated/standing at"), nunca de movimiento ni de contraste con el origen.
   - **Gesto contenido cuando hay subZoneNote**: un gesto corporal muy expresivo/dominante (reírse con la cabeza hacia atrás, brazos en movimiento amplio) compite con la instrucción de reubicación y el generador de imagen tiende a ignorar la sub-zona, quedándose en el lugar de la imagen de referencia original. Si el shot tiene \`subZoneNote\`, preferí un gesto MÁS CONTENIDO (sostener la copa, mirada hacia el trago, sonrisa suave, mano cerca del rostro) — reservá los gestos expresivos (risas francas, brazos en movimiento) para shots SIN subZoneNote. Si el shot realmente necesita un gesto expresivo Y sub-zona a la vez, usa la afirmación de estado corta descrita arriba (sin mencionar el origen) para maximizar la tasa de éxito.
